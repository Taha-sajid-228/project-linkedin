...
class User(Base):
    ...
    conversation_participations = relationship(
        "ConversationParticipant", foreign_keys="ConversationParticipant.user_id",
...
class Follow(Base):
    ...
    __table_args__ = (
        ...
        Index("index_follows_follower_id", "follower_id"),

        # Makes following-list queries faster.
        Index("index_follows_following_id", "following_id"),
    )
...
class Friendship(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("sender_id", "receiver_id", name="unique_friendship_sender_receiver"),

        # A user cannot send a friend request to themselves.
        CheckConstraint("sender_id != receiver_id", name="check_friendship_not_self"),

        # Only these three statuses are allowed.
        CheckConstraint("status IN ('pending', 'accepted', 'rejected')", name="check_friendship_valid_status"),

        # Makes sent request queries faster.
        Index("index_friendships_sender_id", "sender_id"),

        # Makes received request queries faster.
        Index("index_friendships_receiver_id", "receiver_id"),

        # Makes queries filtered by status faster.
        Index("index_friendships_status", "status"),
    )
...
class ConversationParticipant(Base):
    ...
    user = relationship("User", foreign_keys=[user_id], back_populates="conversation_participations")
    ...
    __table_args__ = (
        ...
        Index("index_conversation_participants_conversation_id", "conversation_id"),
        Index("index_conversation_participants_user_id", "user_id"),
    )
...
class Message(Base):
    ...
    __table_args__ = (
        Index("index_messages_conversation_id", "conversation_id"),
        Index("index_messages_sender_id", "sender_id"),

        # Useful for loading messages in chronological order.
        Index("index_messages_conversation_created_at", "conversation_id", "created_at"),

        # Useful for unread message queries.
        Index("index_messages_conversation_read_status", "conversation_id", "is_read"),
    )
...
class Post(Base):
    ...
    original_post = relationship(
        "Post", remote_side=[id],
        foreign_keys=[original_post_id], back_populates="reposts"
    )

    # From an original post, access all of its reposts:
    # original_post.reposts
    reposts = relationship("Post", foreign_keys=[original_post_id], back_populates="original_post")
